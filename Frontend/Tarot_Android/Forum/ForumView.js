...
const coloredTags = tags.map((tag, index) => ({ name: tag, color: tagColors[index % tagColors.length] }));

const samplePosts = [
  { id: 1, tag: '爱情', author: '星尘塔罗', title: '爱情塔罗解读', content: '今天抽到的是恋人牌，意味着感情关系将进入新阶段，单身的朋友可能会遇到心仪的对象，有伴侣的朋友感情会更加深厚...', time: '2小时前', likes: 24, comments: 8 },
  { id: 2, tag: '学业', author: '神秘小猫', title: '塔罗新手求问', content: '大家觉得学习塔罗要先从哪套牌开始呢？经典韦特还是现代塔罗牌？求推荐适合新手的入门牌卡...', time: '5小时前', likes: 15, comments: 12 },
  { id: 3, tag: '事业', author: '职场占卜师', title: '事业运势解读', content: '本周抽到的是战车牌，预示着需要主动出击把握机会。在职场上可能会有新的挑战，但正是你展现实力的好时机...', time: '7小时前', likes: 32, comments: 5 },
  { id: 4, tag: '财富', author: '财富密码', title: '财务规划建议', content: '圣杯九逆位提示要审视当前的投资计划，避免冲动消费。建议做长远规划，寻求专业理财建议...', time: '9小时前', likes: 18, comments: 7 },
  { id: 5, tag: '灵性成长', author: '心灵导师', title: '塔罗冥想方法', content: '分享一种结合塔罗牌的冥想技巧：选择一张牌，聚焦牌面细节，感受能量流动...', time: '昨天', likes: 42, comments: 15 },
  { id: 6, tag: '塔罗解读', author: '解牌大师', title: '三张牌解读实例', content: '今天为一位朋友做了三张牌解读：过去-女祭司，现在-命运之轮，未来-太阳。分享解读思路...', time: '昨天', likes: 29, comments: 11 },
  { id: 7, tag: '塔罗教学', author: '塔罗导师', title: '小阿卡纳解读技巧', content: '如何解读权杖、圣杯、宝剑、星币四组小阿卡纳牌？分享我的教学经验...', time: '2天前', likes: 37, comments: 9 }
];
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.searchContainer}>
        ...
        <TouchableOpacity style={styles.filterButton} onPress={() => console.log('打开筛选菜单')}>
          ...
          <TouchableOpacity key={post.id} style={styles.postCard} onPress={() => navigation.navigate('PostDetail', { post })}>
            ...
            <View style={styles.postFooter}>
              <View style={styles.authorContainer}>
                <View style={styles.authorIcon}>
                  <Ionicons name="person-circle" size={16} color={colors.subtext} />
                </View>
            ...
            <View style={styles.statsContainer}>
              <View style={styles.statItem}>
                <Ionicons name="heart-outline" size={16} color={colors.accentGold} />
                <Text style={styles.statText}>{post.likes}</Text>
              </View>
              <View style={styles.statItem}>
                <Ionicons name="chatbubble-ellipses-outline" size={16} color={colors.accentGold} />
      ...
      <NewPostModal visible={showNewPost} onClose={() => setShowNewPost(false)} onPublish={handlePublish} />
    </SafeAreaView>
  );
}
...
const getStyles = (c) =>
  StyleSheet.create({
    ...
    tagScrollContent: { paddingHorizontal: 16, flexDirection: 'row' },
    tagButton: {
      backgroundColor: c.surfaceCard,
      paddingVertical: 8,
      paddingHorizontal: 16,
    ...
    selectedTagText: { color: c.textInverse, fontWeight: 'bold' },
    postsContainer: { padding: 16, paddingBottom: 100 },
    postCard: {
      backgroundColor: c.surfaceCard,
      padding: 16,
      borderRadius: 16,
    ...
    postTag: { color: c.subtext, fontSize: 14, fontWeight: '500' },
    postTitle: { color: c.text, fontSize: 20, fontWeight: 'bold', marginBottom: 8 },
    postContent: { color: c.subtext, fontSize: 16, lineHeight: 22, marginBottom: 12 },
    ...
    authorIcon: { marginRight: 8 },
    postAuthor: { color: c.subtext, fontSize: 14, fontWeight: '500' },
    postTime: { color: c.textMuted, fontSize: 14, fontStyle: 'italic' },
    statsContainer: {
      flexDirection: 'row',
      marginTop: 12,
      borderTopWidth: 1,
