...
export const AudioProvider = ({ children }) => {
  ...
  // 装载并播放
  const playNewAudio = useCallback(async (source) => {
    try {
      setIsLoading(true);
      setError(null);
      ...
      // 关键：把回调直接传给 createAsync，并设置更新频率
  ...
  const skipForward = useCallback(async (milliseconds = 15000) => {
    if (!sound) return;
    try {
  ...
  const skipBackward = useCallback(async (milliseconds = 15000) => {
    if (!sound) return;
    try {
